...
export async function previewCommand(options) {
    ...
    try {
        execSync(`"${loInfo.path}" --headless --convert-to pdf --outdir "${previewDir}" "${tempPptxPath}"`, { timeout: 120000, stdio: "pipe" });
    ...
    try {
        ...
        if (hasPdftoppm) {
            execSync(`pdftoppm -png -r ${dpi} "${pdfPath}" "${path.join(previewDir, "slide")}"`, { timeout: 120000, stdio: "pipe" });
        }
        else {
            // Try pdftocairo as alternative
            const hasPdftocairo = checkCommand("pdftocairo");
            if (hasPdftocairo) {
                execSync(`pdftocairo -png -r ${dpi} "${pdfPath}" "${path.join(previewDir, "slide")}"`, { timeout: 120000, stdio: "pipe" });
            }
            else {
