...
export async function initCommand(
  ...
  try {
    ...
    if (fs.existsSync(outputDir)) {
      if (!options.force) {
        console.error(
          chalk.red(`Error: Directory already exists: ${outputDir}`)
        );
    ...
    console.log();
    console.log(
      chalk.green(`✓ Created '${projectName}' with ${countFiles(outputDir)} files`)
    );
    console.log();
...
function extractPresentationInfo(workDir: string): PresentationInfo {
